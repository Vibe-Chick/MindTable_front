...
| `/restaurants` | 그룹 조건(위치·예산) 입력 → 제휴 식당 / 지도 fallback (리스트·지도 탭) → 투표 과반 확정 | 식당 추천 |
| `/subscription` | 무료 매칭 소진 후 구독 유도 | 수익 모델 |
| `/review/:matchId` | 식사 후 리뷰 (자유서술 3 + 강제선택 1) → Big Five 델타 · 다양성 선호도 보정 결과 | 보정 |
| `/mypage` | 계정 설정 · 개인 설정 · 통계/매칭 기록 | 마이페이지 플로우 |
| `/notifications` | 알림 센터 (매칭 완료 · 장소 확정 · 리뷰 요청 · 학교 인증), 메인 벨 배지 | 매칭 · 식당 · 보정 |
