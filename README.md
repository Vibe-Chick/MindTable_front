...
| `/login` | 카카오 로그인 / 이메일 로그인 | 로그인 플로우 |
| `/signup` `/find-password` | 이메일 회원가입 · 비밀번호 찾기(코드 인증) | 로그인 플로우 |
| `/oauth/kakao` | 카카오 인가 코드 콜백 (실서버 연동 시) | 로그인 플로우 |
| `/verify-school` | 학교 인증: 대학 이메일 코드 인증 → 학교(도메인 자동 추정)·전공 확정 | 로그인 플로우 |
| `/home` | 메인 (매칭 신청 진입) | — |
...
로그인 후 이동은 `nextRouteFor(user)` 기준: 학교 미인증 → `/verify-school`, 성향 테스트 미완료 → `/test`, 완료 → `/home`.
카카오·이메일 어느 쪽으로 가입했든 학교 인증 전에는 매칭을 신청할 수 없습니다.

## 카카오 로그인 연동

`.env`의 `VITE_KAKAO_JS_KEY`에 카카오 개발자 콘솔의 JavaScript 키를 넣고 `VITE_USE_MOCK=false`로 두면,
카카오 SDK로 인가 코드를 받아 `/oauth/kakao`로 돌아온 뒤 백엔드 `POST /auth/kakao { code }`에 넘깁니다.
카카오 콘솔의 Redirect URI에 `http://localhost:3000/oauth/kakao`를 등록해야 합니다.
